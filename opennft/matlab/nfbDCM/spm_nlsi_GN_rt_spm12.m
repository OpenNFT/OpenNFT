function [Ep,Cp,Eh,F,RT,L,dFdp,dFdpp] = spm_nlsi_GN_rt_spm12(M,U,Y)
...
%yk: adopted by Yury Koush for neurofeedback, Koush et al., Cerebral Cortex 2015
% To match default settings that were used at SPM8, keep this flag TRUE,
% see comments in testAnalizeDCMs.m
isMatchSPM8 = 1;
...
for k = 1:M.Nmax
    ...
    try
        ...
        %yk
        if M.rt            
            tmp_time_k = toc(tStart); 
            RT.Iter = k;
            RT.time(k) = tmp_time_k;          
            fprintf(' actual: %.3e (%.2f sec)\n',full(F - C.F),tmp_time_k) 
        end
        %yk
        if ~M.noprint
