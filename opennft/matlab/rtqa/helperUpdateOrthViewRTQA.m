function helperUpdateOrthViewRTQA(cursorPos, flagsPlanes, bgType)
% Function to update orthogonal viewer for RTQA.
%
% input:
...
strParam.modeDispl = [1 0];

displSNR.vol = snrRaw;
displSNR.mat = matTemplMotCorr;
[backg_imgt,backg_imgc,backg_imgs, snr_imgt, snr_imgc, snr_imgs, P] = redrawall(displBackgr.vol, displBackgr.mat, ROIsOverlay, displSNR, P);
...
snr_imgt = uint8(snr_imgt / max(snr_imgt(:)) * 255);
snr_imgc = uint8(double(snr_imgc) / max(snr_imgc(:)) * 255);
snr_imgs = uint8(double(snr_imgs) / max(snr_imgs(:)) * 255);

assignin('base', 'snr_imgt', snr_imgt);
assignin('base', 'snr_imgc', snr_imgc);
assignin('base', 'snr_imgs', snr_imgs);
...
l2 = prod(size(snr_imgc));
m_out.Data(l1+1:l1+l2) = snr_imgc(:);
l3 = prod(size(snr_imgs));
m_out.Data(l1+l2+1:l1+l2+l3) = snr_imgs(:);

assignin('base', 'helperP', P);



function [backg_imgt, backg_imgc, backg_imgs, snr_imgt, snr_imgc, snr_imgs, P] = redrawall(Vol, mat, ROIs, SNR, P)
global strParam
...
% Calculate SNR
[snr_imgt, snr_imgc, snr_imgs] = getOrthVol(coordParam, SNR.vol, SNR.mat);
