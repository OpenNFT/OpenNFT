function [Ep,Cp,Eh,F,RT] = spm_nlsi_GN_rt_spm8(M,U,Y)
...
dFdh  = zeros(nh,1);
dFdhh = zeros(nh,nh);
for k = 1:30 %yk VC NF:44, EMO NF: 30, ELSE default: 128
    ...
    RT.dF(k) = full(dF); % yk    
    criterion = [(dF < 1e-2) criterion(1:end - 1)]; % yk: convergence criteria here
    RT.criterion(k,:) = criterion; % yk
    if all(criterion),
        fprintf(' convergence\n'), break, 
    end
