function [DCM,RT] = spm_dcm_estimate_rt_spm12(P)
...
if ~DCM.options.stochastic
    ...
    %----------------------------------------------------------------------
    %yk
    M.rt = 1; % flag for neurofeedback changes in spm_nlsi_GN_rt
    M.Nmax = 30; % Koush et al., Cerebral Cortex 2015
    M.noprint = 1;
    M.nograph = 1;
    %yk
    [Ep,Cp,Eh,F,RT] = spm_nlsi_GN_rt_spm12(M,U,Y);
